...
window.addEventListener('scroll', () => {
  ...
  document.querySelectorAll('section[id]').forEach(s => {
    ...
    if (link) {
      link.style.color = (s.offsetTop <= y && s.offsetTop + s.offsetHeight > y) ? '#F5A800' : '';
    }
  });
});
...
const barObserver = new IntersectionObserver((entries) => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      e.target.querySelectorAll('.tbar-fill').forEach(bar => bar.classList.add('animate'));
    }
  });
}, { threshold: 0.25 });
